...
async function checkWeather(city, lat = null, lon = null) {
    ...
    if (lat && lon) {
        currentUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric&lang=ru`;
        forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric&lang=ru`;
    } else {
        if (!city) return;
        currentUrl = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric&lang=ru`;
        forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${apiKey}&units=metric&lang=ru`;
    }
...
function getLocation() {
    if (navigator.geolocation) {
        cityInput.placeholder = "Определяем локацию...";
        navigator.geolocation.getCurrentPosition(
            ...
            () => {
                alert("Не удалось получить доступ к геопозиции.");
                cityInput.placeholder = "Введите город...";
            }
        );
    } else {
...
function displayCurrent(data) {
    ...
    let bg = "#1a1a1a";
    if (weather === 'Clear') bg = "linear-gradient(180deg, #56CCF2, #2F80ED)";
...
function displayForecast(data) {
    ...
    daily.forEach(day => {
        ...
        list.innerHTML += `<div class="forecast-item">
...
// Авто-поиск при загрузке (по желанию можно включить getLocation())
